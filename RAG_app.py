...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        ...
        if llm_chooser == list_LLM_providers[0]:
            expander_model_parameters(
                ...
                list_models=["gemini-flash-latest", "gemini-pro-latest"],
            )
    ...
    with tab_open_vectorstore:
        ...
        # User manual input for vector store directory
        folder_path = st.text_input(
            label="**Enter the full path to your existing Vectorstore:**",
            placeholder="e.g., C:/path/to/vectorstore",
            key="vectorstore_path_input"
        )
        
        if folder_path and os.path.isdir(folder_path):
             st.session_state.selected_vectorstore_name = folder_path
        elif folder_path:
             st.error("Invalid directory path. Please enter a valid path.")
             st.session_state.selected_vectorstore_name = ""

        # clicked = st.button("Vectorstore chooser") # Removed deprecated button
        clicked = False 
        if folder_path:
            clicked = True
...
def select_embeddings_model():
    ...
    if st.session_state.LLM_provider == "Google":
        # Use local embeddings to avoid "RESOURCE_EXHAUSTED" (429) errors from Google API
...
def create_memory(model_name="gpt-3.5-turbo", memory_max_token=None):
    ...
    # SIMPLIFIED MEMORY: Always use ConversationBufferMemory to avoid extra API calls
    # that trigger OpenAI quota errors during summarization.
...
def get_response_from_LLM(prompt):
    ...
    try:
        ...
        if "insufficient_quota" in error_msg or "429" in error_msg:
             st.error("OpenAI API quota exceeded. Please check your billing or switch to Google/HuggingFace.")
        else:
             st.warning(e)
...
def chatbot():
    ...
    if prompt := st.chat_input():
        ...
        if "chain" not in st.session_state:
            st.info("Please create or load a vector store to start chatting.")
            st.stop()
